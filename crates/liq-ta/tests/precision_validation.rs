...
use liq_ta::indicators::{
    bollinger, cci, mfi, obv, roc, rocp, rocr, rocr100, rsi, sma, stochastic, var, vwap, williams_r,
};
...
fn report_and_check<F>(name: &str, actual: &[f64], expected: &[f64], tolerance_check: F) -> bool
where
    F: Fn(f64, f64) -> bool,
{
...
fn precision_bollinger_random_walk() {
    ...
    let ref_result = with_precision_mode(PrecisionMode::Fast, || {
        bollinger(&data_f64, 20, 2.0).unwrap()
    });

    let result = with_precision_mode(PrecisionMode::High, || {
        bollinger(&data_f32, 20, 2.0).unwrap()
    });
    ...
    let passed_upper =
        report_and_check("Bollinger/upper", &upper_f64, &ref_result.upper, |a, e| {
            within_hybrid(a, e, 1e-5, 1e-7)
        });
    let passed_middle = report_and_check(
        "Bollinger/middle",
        &middle_f64,
        &ref_result.middle,
    ...
    let passed_lower =
        report_and_check("Bollinger/lower", &lower_f64, &ref_result.lower, |a, e| {
            within_hybrid(a, e, 1e-5, 1e-7)
        });
...
fn precision_roc_random_walk() {
    ...
    let ref_rocr100 = with_precision_mode(PrecisionMode::Fast, || rocr100(&data_f64, 10).unwrap());
    let result_rocr100 =
...
fn precision_obv_random_walk() {
    ...
    let reference = with_precision_mode(PrecisionMode::Fast, || obv(&close, &volume).unwrap());

    let result = with_precision_mode(PrecisionMode::High, || {
        obv(&close_f32, &volume_f32).unwrap()
    });
    let result_f64: Vec<f64> = result.iter().map(|&x| x as f64).collect();
...
fn precision_summary_all_indicators() {
    ...
    // SMA
    {
        ...
        let result = with_precision_mode(PrecisionMode::High, || sma(&data_f32, 20).unwrap());
        let result_f64: Vec<f64> = result.iter().map(|&x| x as f64).collect();
        all_passed &= report_and_check("SMA", &result_f64, &reference, |a, e| {
            within_hybrid(a, e, 1e-5, 1e-7)
        });
    }
    ...
    assert!(all_passed, "One or more precision validation tests failed");
}
