...
fn run_single_series_suite(data: &[f64]) {
    ...
    assert_nan_at_all(
        &[&stochrsi_result.fastk, &stochrsi_result.fastd],
        "stochrsi",
    );
...
fn run_multi_series_suite(
    ...
    assert_nan_at_all(
        &[&boll_out.upper, &boll_out.middle, &boll_out.lower],
        "bollinger",
    );
    ...
    assert_nan_at_all(&[&adx_out.adx, &adx_out.plus_di, &adx_out.minus_di], "adx");
    ...
    assert_nan_at(
        &adosc(&high, &low, &close, &volume, 3, 10).unwrap(),
        "adosc",
    );
    ...
    assert_nan_at(
        &linearreg_intercept(&data0, 5).unwrap(),
        "linearreg_intercept",
    );
