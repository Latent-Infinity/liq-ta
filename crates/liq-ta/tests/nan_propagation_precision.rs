...
fn nan_propagation_mfi() {
    ...
    assert_nan_both_modes("MFI", || mfi(&high, &low, &close, &volume, 14).unwrap());
    assert_nan_pattern_identical("MFI", || mfi(&high, &low, &close, &volume, 14).unwrap());
}
...
fn signed_zero_propagation() {
    ...
    let sma_pos_fast = with_precision_mode(PrecisionMode::Fast, || sma(&data_pos, 5).unwrap());
    let sma_neg_fast = with_precision_mode(PrecisionMode::Fast, || sma(&data_neg, 5).unwrap());
    let sma_pos_high = with_precision_mode(PrecisionMode::High, || sma(&data_pos, 5).unwrap());
    let sma_neg_high = with_precision_mode(PrecisionMode::High, || sma(&data_neg, 5).unwrap());
...
fn nan_at_start_propagation() {
    ...
    assert!(
        sma_fast[0].is_nan(),
        "SMA Fast: NaN at start should propagate"
    );
    assert!(
        sma_high[0].is_nan(),
        "SMA High: NaN at start should propagate"
    );
