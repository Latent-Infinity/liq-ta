...
//!
//! ```
//! use liq_ta::indicators::tema::tema;
//!
...
use crate::traits::{SeriesElement, ValidatedInput, validate_period};
...
///
/// ```
/// use liq_ta::indicators::tema::tema_lookback;
///
...
///
/// ```
/// use liq_ta::indicators::tema::tema_min_len;
///
...
pub fn tema<T: SeriesElement>(data: &[T], period: usize) -> Result<Vec<T>> {
    ...
    let mut result = if TypeId::of::<T>() == TypeId::of::<f64>() {
        ...
        unsafe {
            v.set_len(data.len());
        } // Safe: we write all elements below
        v
...
///
/// ```
/// use liq_ta::indicators::tema::tema_into;
///
/// let mut data: Vec<f64> = Vec::with_capacity(15);
/// for x in 1..=15 {
///     data.push(x as f64);
...
pub fn tema_into<T: SeriesElement>(data: &[T], period: usize, output: &mut [T]) -> Result<usize> {
    ...
    let mut ema1 = if TypeId::of::<T>() == TypeId::of::<f64>() {
        ...
        unsafe {
            v.set_len(data.len());
        } // Safe: ema_into writes all elements
        v
    } else if TypeId::of::<T>() == TypeId::of::<f32>() {
        let mut v: Vec<T> = Vec::with_capacity(data.len());
        unsafe {
