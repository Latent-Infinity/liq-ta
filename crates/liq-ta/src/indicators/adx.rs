...
///
/// ```
/// use liq_ta::indicators::adx::adx_lookback;
///
...
///
/// ```
/// use liq_ta::indicators::adx::adx_min_len;
///
...
///
/// ```
/// use liq_ta::indicators::adx::di_lookback;
///
...
///
/// ```
/// use liq_ta::indicators::adx::di_min_len;
///
...
///
/// ```
/// use liq_ta::indicators::adx::adx_into;
///
/// let high = vec![48.70_f64, 48.72, 48.90, 48.87, 48.82, 49.05, 49.20, 49.35, 49.92, 50.19, 50.12];
/// let low = vec![47.79_f64, 48.14, 48.39, 48.37, 48.24, 48.64, 48.94, 48.86, 49.50, 49.87, 49.20];
/// let close = vec![48.16_f64, 48.61, 48.75, 48.63, 48.74, 49.03, 49.07, 49.32, 49.91, 50.13, 49.53];
...
#[inline]
fn clamp_unit_percent<T: SeriesElement>(value: T) -> T {
    if value.is_finite() {
        let zero = T::zero();
...
fn compute_adx_core<T: SeriesElement>(
    ...
    let plus_di_val = if nan_active {
        ...
        clamp_unit_percent(hundred * smoothed_plus_dm / smoothed_tr)
    } else {
        T::zero()
    };
    let minus_di_val = if nan_active {
        ...
        clamp_unit_percent(hundred * smoothed_minus_dm / smoothed_tr)
    } else {
        T::zero()
    };
    ...
    let first_dx = if !plus_di_val.is_finite() {
        ...
        if di_sum > T::zero() {
            clamp_unit_percent(hundred * di_diff / di_sum)
        } else {
            T::zero()
        }
    };
    ...
    let mut prev_adx = clamp_unit_percent(dx_sum / period_t);
    ...
    for i in (2 * period)..n {
        ...
        let adx_val = clamp_unit_percent((dx - prev_adx).mul_add(alpha, prev_adx));
        adx_out[i] = adx_val;
        prev_adx = adx_val;
    }
