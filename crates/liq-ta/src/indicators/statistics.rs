...
#[inline]
fn clamp_negative_variance_f64(output: &mut [f64], lookback: usize) {
...
#[inline]
fn clamp_negative_variance<T: SeriesElement>(output: &mut [T], lookback: usize) {
    let zero = T::zero();
    for value in &mut output[lookback..] {
...
fn var_talib_f64(data: &[f64], period: usize, output: &mut [f64]) -> Result<()> {
    ...
    while i < unroll_limit {
        ...
        output[i] = (sum_x2 * period_f64 - sum_x_sq) * inv_period_sq;
...
fn var_welford_slow<T: SeriesElement>(data: &[T], period: usize, output: &mut [T]) -> Result<()> {
    ...
    for i in period..n {
        ...
        if nan_count == 0 {
            ...
            let mean_shifted_sq = mean_shifted * mean_shifted;
            output[i] = sum_sq / period_t - mean_shifted_sq;
        } else {
            output[i] = T::nan();
        }
    }
...
pub fn var_into<T: SeriesElement + 'static>(
    ...
    if use_f64_precision::<T>() {
        var_f64_precision(data, period, output)?;
        clamp_negative_variance(output, lookback);
        return Ok(());
    }
    ...
    if TypeId::of::<T>() == TypeId::of::<f64>() {
        ...
        var_talib_f64(data_f64, period, output_f64)?;
        clamp_negative_variance_f64(output_f64, lookback);
        return Ok(());
    }
...
fn var_f64_precision<T: SeriesElement>(data: &[T], period: usize, output: &mut [T]) -> Result<()> {
    ...
    if period == 1 {
        for i in 0..n {
            ...
            output[i] = if val.is_nan() { T::nan() } else { T::zero() };
        }
        return Ok(());
    }
...
fn var_f64_precision_fast<T: SeriesElement>(
    ...
    for i in period..n {
        ...
        let mean_shifted_sq = mean_shifted * mean_shifted;
        let variance = sum_sq / period_f64 - mean_shifted_sq;
...
pub fn zscore_into<T: SeriesElement>(data: &[T], period: usize, output: &mut [T]) -> Result<()> {
    ...
    let variance = (sum_sq / period_t) - mean_sq;
...
/// - The period is invalid (`Error::InvalidPeriod`)
/// - There is insufficient data for the lookback (`Error::InsufficientData`)
pub fn correl<T: SeriesElement + 'static>(
    data0: &[T],
    data1: &[T],
    period: usize,
) -> Result<Vec<T>> {
...
/// - The period is invalid (`Error::InvalidPeriod`)
/// - There is insufficient data for the lookback (`Error::InsufficientData`)
pub fn beta<T: SeriesElement + 'static>(data0: &[T], data1: &[T], period: usize) -> Result<Vec<T>> {
    use std::any::TypeId;
...
pub fn linearreg_slope_into<T: SeriesElement>(
    ...
    // Compute first output (slope b)
    let numerator = period_t * xy_sum - x_sum * y_sum;
    output[lookback] = numerator / denominator_const;
    ...
    for i in (lookback + 1)..n {
        ...
        // Compute slope b
        let numerator = period_t * xy_sum - x_sum * y_sum;
        output[i] = numerator / denominator_const;
    }
...
pub fn linearreg_slope<T: SeriesElement>(data: &[T], period: usize) -> Result<Vec<T>> {
    ...
    // Wrapper optimization: uninitialized allocation for f64/f32 (§5.4)
    // linearreg_slope_into() writes all elements, so this avoids double-write tax
    if TypeId::of::<T>() == TypeId::of::<f64>() {
        let data_f64: &[f64] = unsafe { std::mem::transmute(data) };
        let mut output: Vec<f64> = Vec::with_capacity(data.len());
        unsafe {
        ...
        linearreg_slope_into(data_f64, period, &mut output)?;
        Ok(unsafe { std::mem::transmute(output) })
    } else if TypeId::of::<T>() == TypeId::of::<f32>() {
        let data_f32: &[f32] = unsafe { std::mem::transmute(data) };
        let mut output: Vec<f32> = Vec::with_capacity(data.len());
        ...
        linearreg_slope_into(data_f32, period, &mut output)?;
        Ok(unsafe { std::mem::transmute(output) })
    } else {
        // Generic fallback: safe initialization
        let mut output = vec![T::nan(); data.len()];
...
pub fn tsf_into<T: SeriesElement>(data: &[T], period: usize, output: &mut [T]) -> Result<()> {
    ...
    let b = numerator / denominator_const;
    let a = (y_sum - b * x_sum) / period_t;
    output[lookback] = a + b * x_forecast;
    ...
    for i in (lookback + 1)..n {
        ...
        // Compute regression parameters and forecast
        let numerator = period_t * xy_sum - x_sum * y_sum;
        let b = numerator / denominator_const;
        let a = (y_sum - b * x_sum) / period_t;
        output[i] = a + b * x_forecast;
    }
...
pub fn tsf<T: SeriesElement>(data: &[T], period: usize) -> Result<Vec<T>> {
    ...
    // Wrapper optimization: uninitialized allocation for f64/f32 (§5.4)
    // tsf_into() writes all elements, so this avoids double-write tax
    if TypeId::of::<T>() == TypeId::of::<f64>() {
        let data_f64: &[f64] = unsafe { std::mem::transmute(data) };
        let mut output: Vec<f64> = Vec::with_capacity(data.len());
        unsafe {
        ...
        tsf_into(data_f64, period, &mut output)?;
        Ok(unsafe { std::mem::transmute(output) })
    } else if TypeId::of::<T>() == TypeId::of::<f32>() {
        let data_f32: &[f32] = unsafe { std::mem::transmute(data) };
        let mut output: Vec<f32> = Vec::with_capacity(data.len());
        ...
        tsf_into(data_f32, period, &mut output)?;
        Ok(unsafe { std::mem::transmute(output) })
    } else {
        // Generic fallback: safe initialization
        let mut output = vec![T::nan(); data.len()];
