...
pub fn mama_full_into<T: SeriesElement>(
    ...
    for i in 6..n {
        ...
        smooth[i] = (c1 * data[i] + c2 * data[i - 1] + c3 * data[i - 2] + c4 * data[i - 3]) / c_sum;
        ...
        // Detrender
        detrender[i] = (a * smooth[i] + b * smooth[i - 2] - b * smooth[i - 4] - a * smooth[i - 6])
            * adj_prev_period;

        // Compute InPhase and Quadrature components
        q1[i] =
            (a * detrender[i] + b * detrender[i - 2] - b * detrender[i - 4] - a * detrender[i - 6])
                * adj_prev_period;
        i1[i] = detrender[i - 3];
