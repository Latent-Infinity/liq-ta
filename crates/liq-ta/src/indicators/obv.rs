...
///
/// ```
/// use liq_ta::indicators::obv::obv_lookback;
///
...
///
/// ```
/// use liq_ta::indicators::obv::obv_min_len;
///
...
///
/// ```
/// use liq_ta::indicators::obv::obv_into;
///
/// let close = vec![10.0_f64, 11.0, 10.5, 11.5, 11.0];
/// let volume = vec![100.0, 150.0, 120.0, 200.0, 130.0];
...
fn compute_obv_core<T: SeriesElement + 'static>(close: &[T], volume: &[T], output: &mut Vec<T>) {
    ...
    if TypeId::of::<T>() == TypeId::of::<f64>() {
        ...
        // Cast back to Vec<T>
        let result = unsafe { std::mem::transmute::<Vec<f64>, Vec<T>>(temp) };
        *output = result;
        return;
    }
...
fn compute_obv_core_f64<T: SeriesElement>(
    ...
    output.push(if nan_mode {
        T::nan()
    } else {
        T::from_f64(obv_accum)?
    });
