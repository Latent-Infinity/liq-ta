...
fn should_regenerate() -> bool {
    std::env::var("REGENERATE_GOLDEN").map_or(false, |v| v == "1" || v.to_lowercase() == "true")
}
...
fn generate_test_prices_f32(size: usize) -> Vec<f32> {
    generate_test_prices_f64(size).iter().map(|&x| x as f32).collect()
}
...
fn golden_bollinger_f32_fast_mode() {
    ...
    with_precision_mode(PrecisionMode::Fast, || {
        ...
        if let Some(golden) = load_golden("bollinger_middle", "f32") {
            let passed =
                compare_arrays("Bollinger/f32/fast", &middle_f64, &golden, |a, e| {
                    within_hybrid(a, e, 1e-6, 1e-7)
                });
            assert!(passed, "Bollinger f32 golden test failed");
        } else {
            eprintln!(
                "[SKIP] Bollinger/f32/fast: no golden file (run with REGENERATE_GOLDEN=1)"
            );
        }
    });
}
...
fn golden_bollinger_f64_fast_mode() {
    ...
    with_precision_mode(PrecisionMode::Fast, || {
        ...
        if let Some(golden) = load_golden("bollinger_middle", "f64") {
            ...
            eprintln!(
                "[SKIP] Bollinger/f64/fast: no golden file (run with REGENERATE_GOLDEN=1)"
            );
        }
    });
}
...
fn golden_stochastic_f32_fast_mode() {
    ...
    with_precision_mode(PrecisionMode::Fast, || {
        let result = stochastic(&high, &low, &close, 14, 3, 3)
            .expect("Stochastic computation failed");
        ...
        if let Some(golden) = load_golden("stochastic_k", "f32") {
            let passed =
                compare_arrays("Stochastic/f32/fast", &k_f64, &golden, |a, e| {
                    within_hybrid(a, e, 1e-6, 1e-7)
                });
            assert!(passed, "Stochastic f32 golden test failed");
        } else {
            eprintln!(
                "[SKIP] Stochastic/f32/fast: no golden file (run with REGENERATE_GOLDEN=1)"
            );
        }
    });
}
...
fn golden_stochastic_f64_fast_mode() {
    ...
    with_precision_mode(PrecisionMode::Fast, || {
        ...
        if let Some(golden) = load_golden("stochastic_k", "f64") {
            ...
            eprintln!(
                "[SKIP] Stochastic/f64/fast: no golden file (run with REGENERATE_GOLDEN=1)"
            );
        }
    });
}
