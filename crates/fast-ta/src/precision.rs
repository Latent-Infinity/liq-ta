//! Precision mode configuration for fast-ta indicators.
//!
...
//! # Overview
//!
//! fast-ta uses a "f64 for state, f32 for storage" strategy. When processing
...
//!   original fast-ta behavior for maximum throughput.
//!
...
//! 3. Environment variable `FAST_TA_PRECISION=fast|high`
...
//! ```rust,ignore
//! // Runtime setting
//! use fast_ta::precision::{set_precision_mode, PrecisionMode};
//! set_precision_mode(PrecisionMode::High);
//!
//! // Or via environment variable
//! // FAST_TA_PRECISION=fast cargo run
//!
//! // Or via Cargo feature
//! // fast-ta = { features = ["precision-fast"] }
//! ```
//!
//! ## Tests
//!
...
pub enum PrecisionMode {
    ...
    /// Fast mode.
    ///
    /// Uses native-type accumulators. Matches original fast-ta behavior for
...
fn ensure_initialized() {
    INIT.call_once(|| {
        ...
        if let Ok(val) = std::env::var("FAST_TA_PRECISION") {
